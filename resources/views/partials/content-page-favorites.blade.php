  ...
  <ul class="cards">
    @while ($favorites->have_posts()) @php $favorites->the_post() @endphp
      <li class="card__wrapper">
        ...
        <div><button class="button button--borderless remove-favorite" data-id="{{ get_the_id() }}" type="button">@svg('delete', 'icon--delete', ['focusable' => 'false', 'aria-hidden' => 'true']) {{ __('Remove', 'coop-library') }} <span class="screen-reader-text">{!! sprintf(__('"%s" from my favorites', 'coop-library'), get_the_title()) !!}</span></button></div>
      </li>
    @endwhile
  </ul>
</div>
